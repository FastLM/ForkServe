...
\rowcolor{tblalt}
 ...
 & FS / APC & \win{$0.89$} & \win{$0.74$} & \win{$0.53$} & \win{$0.76$} \\
\bottomrule
\end{tabular}
\end{table}
...
The spine is flat in $k$; APC grows with the hashed union, so the ratio falls from $0.89$--$0.90$ to $0.53$--$0.55$.}
\label{fig:eval-branch}
\end{figure}

\paragraph{The ratio falls with $k$.}
\Cref{tab:branch,fig:eval-branch} is $(L+\ell)/(L+k\ell)$ on the GPU: the spine stays flat while APC grows, so the ratio moves from $0.89$--$0.90$ at $k{=}2$ to $0.53$--$0.55$ at $k{=}8$ on all three families.
Depth $2$ stays at $0.75$--$0.76$.
ForkServe+ does not move the peak; at $k{=}8$ it drops $28$ branches and cuts fan-out from $307$ to $179$\,ms on Llama-3-8B, from $349$ to $191$\,ms on Mistral-7B, and from $363$ to $194$\,ms on DeepSeek-R1.
Prune off returns Llama fan-out to $247$\,ms and DeepSeek fan-out to $271$\,ms.
...
\rowcolor{tblhead}
\toprule
 & & peak & / APC & acc. & fan-out \\
\midrule
\multirow{3}{*}{R1 MATH}
 & APC & $1863$ & --- & $0.555$ & $1604$ \\
\rowcolor{tblalt}
 & FS & \win{$1391$} & \win{$0.75$} & $0.505$ & $2375$ \\
 & FS+ & \win{$1391$} & \win{$0.75$} & $0.505$ & $1553$ \\
\midrule
\multirow{3}{*}{R1 AIME}
 & APC & $2118$ & --- & $0.068$ & $613$ \\
\rowcolor{tblalt}
 & FS & \win{$1646$} & \win{$0.78$} & $0.096$ & $988$ \\
 & FS+ & \win{$1646$} & \win{$0.78$} & $0.055$ & $670$ \\
\midrule
\multirow{3}{*}{4B MATH}
 & APC & $1937$ & --- & $0.635$ & $1178$ \\
\rowcolor{tblalt}
 & FS & \win{$1521$} & \win{$0.79$} & $0.615$ & $1750$ \\
 & FS+ & \win{$1521$} & \win{$0.79$} & $0.635$ & $1223$ \\
\midrule
\multirow{3}{*}{4B AIME}
 & APC & $2150$ & --- & $0.014$ & $462$ \\
\rowcolor{tblalt}
 & FS & \win{$1734$} & \win{$0.81$} & $0.068$ & $748$ \\
 & FS+ & \win{$1734$} & \win{$0.81$} & $0.055$ & $501$ \\
\bottomrule
\end{tabular}
\end{table}

At $D{=}2048$ the ratio is $0.75$--$0.81$ (\Cref{tab:longcot}).
End-to-end stays with APC on MATH-500 ($433$\,s vs.\ $433$\,s on DeepSeek-R1, $322$\,s vs.\ $328$\,s on Qwen3-4B).
