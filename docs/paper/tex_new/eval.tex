...
Substituting the latest DeepSeek-R1 GSM8K peaks ($761$ APC tokens, $561$ \sys tokens, $n{=}16$, $k{=}4$) into an $8$\,GiB pool in \eqref{eq:theta} gives $76$ concurrent trees for APC and $103$ for \sys.
...
\caption{\textbf{APC versus \sys} on six models ($n{=}40$, $\mathrm{tp}{=}2$).
Each spoke is one family.
(a)~(b)~Peak KV divided by the APC peak, so the outer ring is APC and the inner polygon is \sys.
(c)~GSM8K accuracy on the same scale.
ForkServe+ shares the spine in (a) and (b).}
...
\Cref{fig:eval-radar,fig:eval-multi} shows this pattern: a long trunk on GSM8K gives $0.72$--$0.75$, and a shorter trunk on Game24 gives $0.57$--$0.61$.
