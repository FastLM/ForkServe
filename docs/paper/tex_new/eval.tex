...
\Cref{tab:branch} is $(L+\ell)/(L+k\ell)$ on the GPU: the spine stays flat while APC grows, so the ratio moves from $0.90$ to $0.55$.
Depth $2$ stays at $0.75$--$0.76$.
ForkServe+ does not move the peak; at $k{=}8$ it drops $28$ branches and cuts Llama fan-out from $307$\,ms to $179$\,ms.
Prune off returns that fan-out to $247$\,ms.
The last-line stop is not the decode cut here: the winner already ends inside $D$, so emitted tokens stay near $2500$--$3800$ against APC's $8192$.
A batch of $16$ keeps the ratio at $0.72$--$0.73$.
At $D{=}2048$ the ratio is $0.75$--$0.81$ (DeepSeek MATH-500 $1391/1863$, Qwen3-4B $1521/1937$), end-to-end stays with APC, and ForkServe+ prunes $75$ MATH residuals ($1553$\,ms fan-out vs.\ $2375$\,ms).
