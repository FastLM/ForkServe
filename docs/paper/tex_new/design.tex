...
\Cref{fig:pool} is one fan-out on that pool.
...
The same three structures as a prefix-cache block pool---a request table, a hash index, and a frame free list---hold a tree of aliased pages.
(a)~\texttt{fork} writes $(\pi,\mathrm{alias\_len})$ and increfs trunk frames $0$ and $1$.
Only residual frames $5$, $6$, $8$, and $9$ come off the free list.
$H$ stores full committed blocks; speculative frames stay at $\mathrm{hash}{=}\emptyset$.
(b)~LCP commit of $v^\star$ inserts full page $8$ and leaves partial page $9$ unpublished.
Abort drops $v_1$ and $v_2$ from the refcount, so the free tail gains frames $5$ and $6$ only.
Trunk frames stay live at $\mathrm{ref}{=}2$.
$g{\leftarrow}2$ retires in-flight speculative jobs in $O(1)$.}
\label{fig:pool}
\end{figure*}
...
ForkServe+ stops committed decode at an answer marker via vLLM \texttt{stop}.
