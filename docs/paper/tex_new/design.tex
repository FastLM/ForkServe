...
A branch is an alias over the same three structures as a prefix-cache block pool: a request table, a hash index, and a frame free list.
\emph{Left:} $\sigma_u$ stores $(\mathrm{parent},\mathrm{alias\_len},\rho)$, a mode, and a generation~\textcircled{1}.
\texttt{fork} copies the two words and increfs the trunk, $O(1)$ in $L$, and does not probe $H$~\textcircled{2}.
LCP commit publishes full winner blocks and bumps $g$, so in-flight speculative jobs drop~\textcircled{6}.
\texttt{abort} decrefs residual pages only, leaving the spine $M_{\mathrm{spine}}{=}b(L{+}\ell_\star)$~\textcircled{7}.
\emph{Right:} the trunk is one physical copy whose refcount is the number of live mappers; each child allocates only $\rho$, still at $\mathrm{hash}{=}\emptyset$~\textcircled{3}.
$H$ holds full committed blocks, so a later session can hash-hit the winner~\textcircled{4}.
The free list is doubly linked and stores only $\mathrm{ref}{=}0$ frames; \texttt{fork} pops residual frames and \texttt{abort} splices them back, never the trunk~\textcircled{5}.
A write with $\mathrm{ref}{>}1$ copies the valid rows of that page.}
\label{fig:pool}
\end{figure*}
...
On the GSM8K forest the tree already holds less KV than APC, yet expand is slower ($140$ vs.\ $87$\,ms) because every residual is still prefilled.
The cascade is what removes that work.
...
(b)~APC and stock P/D clone the trunk $k$ ways on the first expand; CoW aliases $L$; \app{} also drops loser $\Work$ and $\Xfer$ before a kernel or a connector write.}
\label{fig:app}
\end{figure*}
...
A score below $\tau$ (default $0.15$) is $\mathrm{DS}$: the branch never starts prefill, and it never becomes a hash key.
...
That is abort expressed as a prefix of the kernel, not a wait for all $k$ residuals to finish.
...
Stock P/D uses \eqref{eq:xfer-stock} and therefore ships every page it computed, including the $k$-way trunk.
\app uses \eqref{eq:xfer}: $\Xfer_i$ is nonzero only for a kept residual whose action is $\mathrm{HP}$ or $\mathrm{PF}$, and then only for the miss tail that was actually computed.
