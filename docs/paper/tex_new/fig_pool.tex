...
\node[pg={fsblue}{fslight}] at (7.55,4.28) {trunk\\[-1pt]\(L\)\\[-1pt]RO};
\node[pg={fsblue}{fslight}] at (8.72,4.28) {\(\cdots\)\\[-1pt]in \(H\)};
\node[pg={fsorange!80!black}{fsspec}] at (10.05,4.28) {\(\rho_i\)\\[-1pt]\(\mathrm{ref}{=}1\)};
\node[pg={fsorange!80!black}{fsspec}] at (11.22,4.28) {\(\cdots\)\\[-1pt]\(\mathrm{hash}{=}\emptyset\)};
\node[pg={fsorange!80!black}{fsspec},densely dashed] at (12.39,4.28) {tail\\[-1pt]\(n{<}P\)};
\node[note] at (10.41,3.48)
  {alias \(x_u\), alloc \(\rho\) only\qquad \(\mathrm{ref}{>}1\): copy valid rows};
% ===================== 4. hash index =====================
...
\node[fmcard=fsgreen,minimum width=4.15cm,minimum height=0.40cm] (hk) at (8.70,2.05)
  {\(h(\mathrm{parent},\,\mathrm{block},\,\mathrm{extra})\rightarrow\mathrm{page}\)};
\node[fmcard=fsorange,minimum width=2.55cm,minimum height=0.40cm] (hu) at (12.55,2.05)
  {spec, partial \(\notin H\)};
...
\node[fp] (fh) at (7.15,0.62) {Head};
\node[fp] (fa) at (8.15,0.62) {free};
\node[fp] (fb) at (9.15,0.62) {\(\cdots\)};
\node[fp] (ft) at (10.15,0.62) {Tail};
\draw[flink] (fh) -- (fa);
\draw[flink] (fa) -- (fb);
\draw[flink] (fb) -- (ft);
\node[note,anchor=west] at (10.70,0.62) {\texttt{fork} pops \(\rho\)\;\(\cdot\)\;\texttt{abort} returns \(\rho\)};
% ===================== cross-boundary =====================
