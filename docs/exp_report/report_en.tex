...
\hypersetup{
  ...
  citecolor=Ink,
  urlcolor=APC,
  pdftitle={ForkServe: Branch-Aware Prefill Pruning for Efficient Agentic LLM Serving},
}
...
\title{\textbf{ForkServe: Branch-Aware Prefill Pruning\\[-0.12em]
for Efficient Agentic LLM Serving}}
\author{%
\begin{tabular}[t]{c}
...
Setting L compares five prefill methods on the same fan-out in the control plane: APC and hash prefill still clone the first fan-out, stock disaggregated prefill adds a full KV transfer, and advanced prefill pruning (APP) is the only method that both shares the trunk and drops low-score residuals before prefill (modeled fan-out $35.9$\,ms vs.\ $139$\,ms for APC).
\end{abstract}
...
APP & method & Advanced prefill pruning on that tree: hash-skip published blocks, drop low-score residuals before GPU prefill, and \texttt{insert} only retained KV into a disaggregated prefill connector. \\
\bottomrule
\end{tabular}
...
Model & Qwen3-14B (A--C, G, H), Qwen3-8B (E, F), \\
      & DeepSeek-R1-Distill-Llama-8B (I, J), \texttt{bfloat16} \\
...
Later sections report A--C in full on Qwen3-14B, then G--H (14B), E--F (8B), and I--J (DeepSeek-R1 8B) on the seven-file math suite (APC vs.\ ForkServe only), then K (ForkServe+ concurrency from those peaks) and L (prefill-method comparison).
...
We do not report that reduction as measured GPU latency.
The concurrency numbers in setting K remain the system-level claim, because they plug measured peaks ($1563$ vs.\ $1163$) into \eqref{eq:conc}.
...
Setting L places APP against hash prefill and disaggregated prefill on the fan-out those GPU runs leave open: hash prefill does not help until blocks are published, disaggregated prefill adds a transfer, and APP is the method whose modeled fan-out is below APC.
...
Confirming that ordering as A100 wall-clock, on the same GSM8K forest, is the remaining measurement.
