...
A second run compares APC and ForkServe on seven Tree-of-Thoughts math files ($n{=}40$--$80$) for both Qwen3-14B and Qwen3-8B, at $\mathrm{tp}{=}2$ and $\mathrm{tp}{=}4$.
...
Generate time at $\mathrm{tp}{=}4$ is $0.61$--$0.66\times$ the two-GPU run and stays within a few percent of APC.
\end{abstract}
...
Model & Qwen3-14B (settings A--C, G, H) and Qwen3-8B (settings E, F), \texttt{bfloat16} \\
...
Tensor parallel $\mathrm{tp}{=}2$ places the model on GPU $0,1$ ($\approx 38497$\,MiB each on the full GSM8K 14B run; $\approx 37381$--$37733$\,MiB on the 14B math suite; $\approx 37305$--$37579$\,MiB on Qwen3-8B).
$\mathrm{tp}{=}4$ uses GPU $0$--$3$ ($\approx 38797$\,MiB each on settings B--C; $\approx 37691$--$37871$\,MiB on the 14B math suite; $\approx 37599$--$37665$\,MiB on Qwen3-8B).
...
Settings E--H additionally score SVAMP and GSM-Hard with the GSM8K numeric extractor, MATH-500 / AIME / AMC23 with boxed contest answers.}
\end{table}
...
Later sections report A--C in full on Qwen3-14B, then G--H (14B) and E--F (8B) on the seven-file math suite (APC vs.\ ForkServe only).
...
Generate time at four GPUs is $0.61$--$0.66\times$ the two-GPU run and stays with APC.
