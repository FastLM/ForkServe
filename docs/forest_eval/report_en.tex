...
A second $\mathrm{tp}{=}2$ run on Qwen3-8B compares APC and ForkServe on seven Tree-of-Thoughts math files ($n{=}40$--$80$).
Peak KV is $0.62$--$0.80\times$ APC on every file; official scores match or exceed APC on six of seven tasks; generate time stays within $3\%$ of APC.
\end{abstract}
...
Model & Qwen3-14B (settings A--C) and Qwen3-8B (setting E), \texttt{bfloat16} \\
...
Game24: $719$ easy / $573$ medium / $70$ hard by published solve rate; success is a valid expression to $24$ (setting E uses decode $512$ on an $80$-item prefix).
...
Setting E additionally scores SVAMP and GSM-Hard with the GSM8K numeric extractor, MATH-500 / AIME / AMC23 with boxed contest answers.}
\end{table}
...
E. Qwen3-8B math suite & $2$ & $0,1$ & $40$--$80$; batches of $8$ & GSM8K, SVAMP, GSM-Hard, MATH-500, AIME, AMC23, Game24 \\
\bottomrule
\end{tabular}
\caption{Configurations and datasets.
Later sections report A--C in full on Qwen3-14B, then E on Qwen3-8B (APC vs.\ ForkServe only).
...
On Qwen3-8B (setting E), seven math files at $\mathrm{tp}{=}2$ give $\mathrm{peak}^{\mathrm{FS}}/\mathrm{peak}^{\mathrm{APC}}\in[0.62,0.80]$, official scores at or above APC on six files, and generate time within $3\%$.
