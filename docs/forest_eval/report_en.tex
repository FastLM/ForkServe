...
\fancyhead[R]{\small\color{Mute}Qwen3-14B / Qwen3-8B}
...
A second run on Qwen3-8B compares APC and ForkServe on seven Tree-of-Thoughts math files ($n{=}40$--$80$) at $\mathrm{tp}{=}2$ and $\mathrm{tp}{=}4$.
Peak KV is $0.62$--$0.80\times$ APC on every file and does not change when the tensor width doubles: peak tracks the committed path, not the number of ranks.
Generate time at $\mathrm{tp}{=}4$ is $0.66\times$ the two-GPU run and stays within $3\%$ of APC.
\end{abstract}
...
Model & Qwen3-14B (settings A--C) and Qwen3-8B (settings E, F), \texttt{bfloat16} \\
...
Tensor parallel $\mathrm{tp}{=}2$ places the model on GPU $0,1$ ($\approx 38497$\,MiB each on Qwen3-14B; $\approx 37305$--$37579$\,MiB each on Qwen3-8B).
$\mathrm{tp}{=}4$ uses GPU $0$--$3$ ($\approx 38797$\,MiB each on Qwen3-14B; $\approx 37599$--$37665$\,MiB each on Qwen3-8B).
...
Settings E and F additionally score SVAMP and GSM-Hard with the GSM8K numeric extractor, MATH-500 / AIME / AMC23 with boxed contest answers.}
\end{table}
...
Later sections report A--C in full on Qwen3-14B, then E and F on Qwen3-8B (APC vs.\ ForkServe only).
...
\section{Qwen3-8B math suite (setting E)}

Setting E repeats the APC vs.\ ForkServe comparison on Qwen3-8B, $\mathrm{tp}{=}2$, GPU $0,1$.
The recipe is Tree-of-Thoughts with $k{=}4$ and batches of eight.
Grade-school files (GSM8K, SVAMP, GSM-Hard, Game24) decode $512$ tokens per problem; contest files (MATH-500, AIME, AMC23) decode $768$.
$n$ is $80$ except AIME ($73$, the local mix) and AMC23 ($40$, the full file).
There is no recompute arm on this run: the question is whether branch-aware abort still sits below hashed APC, at matched official scores, on a smaller model and a wider math mix.

Resident GPU memory is $\approx 37305$\,MiB per rank under APC and $\approx 37423$--$37579$\,MiB under ForkServe.
...
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
Dataset & $n$ & $D$ & APC & ForkServe & $\Delta$ & FS correct \\
\midrule
GSM8K & $80$ & $512$ & $0.925$ ($74$) & $\mathbf{0.963}$ ($77$) & $+3.8$\,pp & $77/80$ \\
SVAMP & $80$ & $512$ & $0.863$ ($69$) & $\mathbf{0.888}$ ($71$) & $+2.5$\,pp & $71/80$ \\
GSM-Hard & $80$ & $512$ & $0.650$ ($52$) & $\mathbf{0.663}$ ($53$) & $+1.3$\,pp & $53/80$ \\
MATH-500 & $80$ & $768$ & $0.638$ ($51$) & $\mathbf{0.650}$ ($52$) & $+1.3$\,pp & $52/80$ \\
AIME & $73$ & $768$ & $0.041$ ($3$) & $0.041$ ($3$) & $0$ & $3/73$ \\
AMC23 & $40$ & $768$ & $0.275$ ($11$) & $\mathbf{0.350}$ ($14$) & $+7.5$\,pp & $14/40$ \\
Game24 & $80$ & $512$ & $\mathbf{0.713}$ ($57$) & $0.700$ ($56$) & $-1.3$\,pp & $56/80$ \\
\bottomrule
\end{tabular}
\caption{Setting E, Qwen3-8B, $\mathrm{tp}{=}2$.
ForkServe matches or beats APC on six of seven files.
The Game24 gap is one problem on an $80$-item easy prefix ($57$ vs.\ $56$).
AIME remains $3/73$ on both stacks: $8$B Tree-of-Thoughts with $D{=}768$ does not solve contest items, so the file is an efficiency check rather than a quality claim.}
\end{table}

On GSM8K the step slices stay aligned and slightly higher for ForkServe:
steps $1$ $4/4$ vs.\ $4/4$,
steps $2$ $27/28$ vs.\ $26/28$,
steps $3$ $18/18$ vs.\ $17/18$,
steps $4{+}$ $28/30$ vs.\ $27/30$.
On MATH-500, level $1$ is $7/8$ on both; levels $4$--$5$ stay the hard tail ($0.53$/$0.33$ vs.\ $0.47$/$0.39$).

\begin{figure}[h]
\centering
\begin{tikzpicture}[x=1.22cm,y=2.55cm]
  \def\w{0.34}
  ...
  \foreach \i/\lab/\apc/\fs in {
    0/GSM8K/0.925/0.9625,
    1/SVAMP/0.8625/0.8875,
    2/GSM-H/0.650/0.6625,
    3/MATH/0.6375/0.650,
    4/AIME/0.0411/0.0411,
    5/AMC23/0.275/0.350,
    6/G24/0.7125/0.700} {
  ...
  \fill[APC] (4.35,1.10) rectangle (4.58,1.18);
  \node[font=\scriptsize,anchor=west] at (4.64,1.14) {APC};
  \fill[FS] (5.45,1.10) rectangle (5.68,1.18);
  \node[font=\scriptsize,anchor=west] at (5.74,1.14) {ForkServe};
  \node[font=\small,anchor=south] at (3.2,1.22) {Official score};
\end{tikzpicture}
\caption{Setting E scores.
ForkServe is the right bar in each pair.
AIME is near zero on both; the remaining files sit with or above APC.}
\end{figure}

\subsection{Storage and latency}

Peak KV is the maximum over sequential batches of eight (five batches on AMC23).
End-to-end time is the summed generate wall-clock of those batches.
...
\toprule
Dataset & APC peak & FS peak & FS/APC & APC e2e (s) & FS e2e (s) & vs.\ APC & APC fan-out (s) \\
\midrule
GSM8K & $1582$ & $\mathbf{1182}$ & $0.75$ & $36.4$ & $35.4$ & $-2.8\%$ & $0.58$ \\
SVAMP & $1359$ & $\mathbf{959}$ & $0.71$ & $35.4$ & $35.7$ & $+1.0\%$ & $0.57$ \\
GSM-Hard & $1462$ & $\mathbf{1062}$ & $0.73$ & $44.1$ & $44.1$ & $-0.1\%$ & $0.58$ \\
MATH-500 & $1765$ & $\mathbf{1349}$ & $0.76$ & $66.5$ & $66.2$ & $-0.4\%$ & $0.58$ \\
AIME & $2118$ & $\mathbf{1702}$ & $0.80$ & $67.4$ & $67.3$ & $-0.1\%$ & $0.56$ \\
AMC23 & $1920$ & $\mathbf{1504}$ & $0.78$ & $33.7$ & $33.5$ & $-0.5\%$ & $0.30$ \\
Game24 & $1289$ & $\mathbf{793}$ & $0.62$ & $44.6$ & $44.3$ & $-0.7\%$ & $0.70$ \\
\bottomrule
\end{tabular}%
}
\caption{Setting E storage and generate time.
Peak is always $\mathrm{peak}^{\mathrm{FS}}<\mathrm{peak}^{\mathrm{APC}}$.
The lowest ratio is Game24 ($0.62$), where the trunk is short ($L{\approx}72$--$74$), so unused residuals are a larger share of the hashed union, as \eqref{eq:gap} predicts.
AIME is the highest ratio ($0.80$) because the trunk is longer.
Generate time stays within $3\%$ of APC; ForkServe fan-out is $0.84$--$1.08\,\mathrm{s}$ vs.\ APC $0.30$--$0.70\,\mathrm{s}$, the same abort/CoW micro-gap as on Qwen3-14B.}
\end{table}

ForkServe fan-out on the same files is $0.95$, $0.86$, $0.85$, $0.94$, $1.08$, $0.55$, $0.84\,\mathrm{s}$.
Per eight-problem batch that is about $85$--$108\,\mathrm{ms}$ vs.\ APC $56$--$70\,\mathrm{ms}$ (AMC23 $110$ vs.\ $60\,\mathrm{ms}$ on five batches).
The extra tens of milliseconds do not move wall-clock once $D\in\{512,768\}$.

\begin{figure}[h]
\centering
\begin{tikzpicture}[x=1.22cm,y=0.00115cm]
  \def\w{0.34}
  ...
  \foreach \i/\lab/\apc/\fs in {
    ...
    \fill[APC] ({\i-\w},0) rectangle ({\i},\apc);
    \fill[FS] ({\i},0) rectangle ({\i+\w},\fs);
    \node[font=\scriptsize,anchor=east,rotate=40] at (\i,-45) {\lab};
  }
  \draw[Ink] (-0.45,0) -- (6.85,0);
  ...
  \node[font=\small,anchor=south] at (3.2,2550) {Peak resident KV (tokens)};
\end{tikzpicture}
\caption{Setting E peak KV.
Every file is below APC.
Game24 shows the largest abort gap; AIME the smallest, still $20\%$ below the hashed union.}
\end{figure}

Decode tokens on the winner path are fewer under ForkServe when generation stops early (GSM8K $21530$ vs.\ APC $40960$; SVAMP $16661$ vs.\ $40960$), which is why GSM8K wall-clock is $2.8\%$ below APC even though fan-out is higher.
MATH-500 and AIME stay near the decode cap on both stacks ($44006$ vs.\ $61440$, $55790$ vs.\ $56064$), so e2e ratios collapse to $\approx 1$.

The storage-efficient serving check \eqref{eq:goal} against APC holds on all seven files.
...
\subsection{Four GPUs (setting F)}

Setting F is the same Qwen3-8B files, prompts, $k$, decode budgets, and batching as setting E, on GPU $0$--$3$ with $\mathrm{tp}{=}4$.
Resident memory is $\approx 37599$\,MiB per rank under APC and $\approx 37665$\,MiB under ForkServe.
Peak KV is identical to setting E on every file (Table below): $M_{\mathrm{spine}}$ is the committed-path length, so tensor width changes $T_{\mathrm{dec}}$, not the storage ratio.
...
\begin{tabular}{@{}lrrrrrrr@{}}
\toprule
Dataset & $n$ & APC & ForkServe & $\Delta$ & APC e2e (s) & FS e2e (s) & vs.\ APC \\
\midrule
GSM8K & $80$ & $0.925$ ($74$) & $\mathbf{0.950}$ ($76$) & $+2.5$\,pp & $25.1$ & $23.5$ & $-6.1\%$ \\
SVAMP & $80$ & $\mathbf{0.863}$ ($69$) & $0.838$ ($67$) & $-2.5$\,pp & $23.2$ & $23.8$ & $+2.4\%$ \\
GSM-Hard & $80$ & $0.638$ ($51$) & $0.638$ ($51$) & $0$ & $29.3$ & $29.2$ & $-0.1\%$ \\
MATH-500 & $80$ & $\mathbf{0.675}$ ($54$) & $0.638$ ($51$) & $-3.8$\,pp & $43.7$ & $43.5$ & $-0.4\%$ \\
AIME & $73$ & $0.027$ ($2$) & $\mathbf{0.041}$ ($3$) & $+1.4$\,pp & $44.2$ & $44.4$ & $+0.5\%$ \\
AMC23 & $40$ & $\mathbf{0.350}$ ($14$) & $0.300$ ($12$) & $-5.0$\,pp & $22.2$ & $22.0$ & $-0.9\%$ \\
Game24 & $80$ & $\mathbf{0.688}$ ($55$) & $0.650$ ($52$) & $-3.8$\,pp & $29.2$ & $29.2$ & $-0.1\%$ \\
\bottomrule
\end{tabular}%
}
\caption{Setting F, Qwen3-8B, $\mathrm{tp}{=}4$.
Peak KV matches setting E ($1182$, $959$, $1062$, $1349$, $1702$, $1504$, $793$ vs.\ APC $1582$, $1359$, $1462$, $1765$, $2118$, $1920$, $1289$).
Official scores stay within three problems of APC.
Generate time is within $3\%$ of APC except GSM8K, where ForkServe is $6\%$ faster because winner decode stops earlier ($21369$ vs.\ $40960$ tokens).}
\end{table}

ForkServe fan-out on setting F is $0.68$, $0.63$, $0.64$, $0.68$, $0.73$, $0.38$, $0.63\,\mathrm{s}$ against APC $0.45$, $0.46$, $0.44$, $0.47$, $0.44$, $0.24$, $0.50\,\mathrm{s}$.
The abort/CoW tax remains a few tenths of a second on a $22$--$44\,\mathrm{s}$ generate.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Dataset & FS e2e, $\mathrm{tp}{=}2$ (s) & FS e2e, $\mathrm{tp}{=}4$ (s) & $\mathrm{tp}{=}4/\mathrm{tp}{=}2$ & peak FS/APC \\
\midrule
GSM8K & $35.4$ & $23.5$ & $0.66$ & $0.75$ \\
SVAMP & $35.7$ & $23.8$ & $0.67$ & $0.71$ \\
GSM-Hard & $44.1$ & $29.2$ & $0.66$ & $0.73$ \\
MATH-500 & $66.2$ & $43.5$ & $0.66$ & $0.76$ \\
AIME & $67.3$ & $44.4$ & $0.66$ & $0.80$ \\
AMC23 & $33.5$ & $22.0$ & $0.66$ & $0.78$ \\
Game24 & $44.3$ & $29.2$ & $0.66$ & $0.62$ \\
\bottomrule
\end{tabular}
\caption{Settings E vs.\ F on ForkServe.
Four-way tensor parallel multiplies decode ranks and scales wall-clock by $0.66$ on every file, in line with the $0.61$--$0.63$ factor on Qwen3-14B in setting C.
The storage ratio is already fixed by abort in setting E and is unchanged at $\mathrm{tp}{=}4$.}
\end{table}

\begin{figure}[h]
\centering
\begin{tikzpicture}[x=1.22cm,y=0.038cm]
  \def\w{0.34}
  \foreach \y in {0,20,40,60} {
    \draw[Rule] (-0.45,\y) -- (6.85,\y);
    \node[font=\scriptsize,Mute,anchor=east] at (-0.52,\y) {\y};
  }
  \foreach \i/\lab/\tpb/\tpf in {
    0/GSM8K/35.4/23.5,
    1/SVAMP/35.7/23.8,
    2/GSM-H/44.1/29.2,
    3/MATH/66.2/43.5,
    ...
    5/AMC23/33.5/22.0,
    6/G24/44.3/29.2} {
    \fill[APC] ({\i-\w},0) rectangle ({\i},\tpb);
    \fill[FS] ({\i},0) rectangle ({\i+\w},\tpf);
    \node[font=\scriptsize,anchor=east,rotate=40] at (\i,-1.6) {\lab};
  }
  \draw[Ink] (-0.45,0) -- (6.85,0);
  \fill[APC] (3.9,72) rectangle (4.13,76);
  \node[font=\scriptsize,anchor=west] at (4.19,74) {$\mathrm{tp}{=}2$};
  \fill[FS] (5.15,72) rectangle (5.38,76);
  \node[font=\scriptsize,anchor=west] at (5.44,74) {$\mathrm{tp}{=}4$};
  \node[font=\small,anchor=south] at (3.2,78) {ForkServe generate time (s)};
\end{tikzpicture}
\caption{Setting E vs.\ setting F, ForkServe only.
The left bar of each pair is two GPUs, the right bar four GPUs.
Every file scales by $0.66$. Peak KV is omitted: it equals the setting-E figure on every file.}
\end{figure}
...
Setting E repeats APC vs.\ ForkServe on Qwen3-8B across seven math files: peak ratios $0.62$--$0.80$, scores with APC, and $T_{\mathrm{e2e}}$ within $3\%$.
Setting F keeps those peak ratios at $\mathrm{tp}{=}4$ and scales ForkServe generate time by $0.66$, still within a few percent of APC.
...
On Qwen3-8B (settings E and F), seven math files give $\mathrm{peak}^{\mathrm{FS}}/\mathrm{peak}^{\mathrm{APC}}\in[0.62,0.80]$ at both $\mathrm{tp}{=}2$ and $\mathrm{tp}{=}4$, with generate time at four GPUs equal to $0.66\times$ the two-GPU run and within $3\%$ of APC on six files.
Where $k{=}4$, $M_{\mathrm{spine}}/M_{\mathrm{APC}}$ sits in $[0.61,0.80]$; where $k{=}2$, the ratio rises to $0.94$, as \eqref{eq:gap} predicts when residuals are large relative to $L$.
